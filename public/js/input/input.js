...
Particle.prototype = {
	...
	toString: function() {
		return 'Particle(' + this._id + ') ' + this._life + ' pos: ' + this._position +
			' vec: ' + this._velocity;
	}
}
...
function simulate(dimensions, options) {
	...
	function go() {
		...
		function add(id, group, position, velocity, size, life, behavior) {
			particles.push(new Particle(id, group, position, velocity, size, life,
				behavior));
		}
		...
		function debug(particle) {
			...
			context.lineTo(particle.position.x + (particle.velocity.x * 10), particle.position
			...
			cohesion: function(range, speed) {
				...
				return function(particle) {
					...
					for (; i < l; i++) {
						...
						if (particles[i] === particle || Vector.distanceSquared(particles[i].position,
			...
			eat: function(food) {
				...
				return function(particle) {
					...
					for (; i < l; i++) {
						...
						if (Vector.distanceSquared(particle.position, neighbour.position) < 2 &&
			...
			attract: function(forceMultiplier, groups) {
				...
				return function(particle) {
					...
					for (; i < l; i++) {
						...
						if (attractor === particle || (grouping && groups.indexOf(attractor.group) ===
								-1)) {
							continue;
						}
